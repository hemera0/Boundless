...
VkInstance VkUtil::CreateInstance(const char** requiredExtensions, const int requiredExtensionsCount) {
...
VkPhysicalDevice VkUtil::GetPhysicalDevice(const VkInstance& instance, const std::vector<const char*>& wantedExtensions) {
...
VkDevice VkUtil::CreateLogicalDevice(const VkInstance& instance, const VkPhysicalDevice& physicalDevice, const VkSurfaceKHR& surface, const std::vector<const char*>& wantedExtensions) {
...
VkSurfaceKHR VkUtil::CreateSurfaceForWindow(const VkInstance& instance, const HWND& hwnd) {
...
bool VkUtil::PhysicalDeviceIsDiscreteGPU(const VkPhysicalDevice& physicalDevice) {
	VkPhysicalDeviceProperties deviceProps{};
	vkGetPhysicalDeviceProperties(physicalDevice, &deviceProps);
	return deviceProps.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU;
}

bool VkUtil::PhysicalDeviceHasExtensions(const VkPhysicalDevice& device, const std::vector<const char*>& wantedExtensions) {
...
uint32_t VkUtil::PhysicalDeviceFindMemoryType(const VkPhysicalDevice& physicalDevice, const uint32_t filter, const VkMemoryPropertyFlags propertyFlags) {
...
VkUtil::QueueFamilyIndices_t VkUtil::FindQueueFamilyIndices(const VkSurfaceKHR& surface, const VkPhysicalDevice& physicalDevice) {
...
VkUtil::SwaphchainSupportData_t VkUtil::QuerySwapchainSupport(const VkSurfaceKHR& surface, const VkPhysicalDevice& physicalDevice) {
...
VkQueue VkUtil::DeviceGetQueueByIndex(const VkDevice& device, uint32_t queueIndex) {
	VkQueue ret = VK_NULL_HANDLE;
	vkGetDeviceQueue(device, queueIndex, 0, &ret);
	return ret;
}

VkCommandPool VkUtil::CreateCommandPool(const VkDevice& device, const VkCommandPoolCreateInfo& createInfo) {
...
VkCommandBuffer VkUtil::CreateCommandBuffer(const VkDevice& device, const VkCommandPool& commandPool) {
...
VkSwapchainKHR VkUtil::CreateSwapchain(const VkDevice& device, const VkPhysicalDevice& physicalDevice, const VkSurfaceKHR& surface, const HWND& windowHandle, VkExtent2D& outExtents) {
	const auto& swapChainSupport = QuerySwapchainSupport(surface, physicalDevice);
	if (swapChainSupport.m_PresentModes.empty() || swapChainSupport.m_SurfaceFormats.empty())
	{
		printf("Device does not have any present modes or surface formats\n");
...
VkSurfaceFormatKHR VkUtil::SwapchainSelectSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR VkUtil::SwapchainSelectPresentMode(const std::vector<VkPresentModeKHR>& availableModes) {
...
VkExtent2D VkUtil::SwapchainGetExtents(const HWND& windowHandle, const VkSurfaceCapabilitiesKHR& capabilities) {
...
VkImageView VkUtil::CreateImageView(const VkDevice& device, const VkImage& image, VkImageViewCreateInfo* createInfo) {
...
VkFormat VkUtil::SwapchainGetImageFormat(const VkPhysicalDevice& physicalDevice, const VkSurfaceKHR& surface) {
	const auto& swapChainSupport = QuerySwapchainSupport(surface, physicalDevice);
	if (swapChainSupport.m_PresentModes.empty() || swapChainSupport.m_SurfaceFormats.empty()) {
		printf("Device does not have any present modes or surface formats\n");
...
void VkUtil::CreateSwapchainImages(const VkDevice& device, const VkSwapchainKHR& swapchain, const VkFormat imageFormat, std::vector<VkImage>& outImages, std::vector<VkImageView>& outImageViews) {
	...
	for (auto i{ 0u }; i < outImageViews.size(); i++) {
		...
		outImageViews[i] = VkUtil::CreateImageView(device, outImages[i], &imageViewCreateInfo);
	}
}

void VkUtil::CommandBufferBegin(const VkCommandBuffer& commandBuffer, VkCommandBufferUsageFlags flags) {
...
void VkUtil::CommandBufferEnd(const VkCommandBuffer& commandBuffer) {
	vkEndCommandBuffer(commandBuffer);
}

void VkUtil::CommandBufferSubmit(const VkCommandBuffer& commandBuffer, const VkQueue& queue, bool wait) {
...
void VkUtil::CommandBufferCopyBufferToImage(const VkCommandBuffer& commandBuffer, const VkBuffer& buffer, const VkImage& image, uint32_t width, uint32_t height) {
...
void VkUtil::CommandBufferBufferBarrier(const VkCommandBuffer& commandBuffer, VkBuffer buffer, VkPipelineStageFlags2 srcStageMask, VkAccessFlags2 srcAccessMask, VkPipelineStageFlags2 dstStageMask, VkAccessFlags2 dstAccessMask)
{

}

void VkUtil::CommandBufferBeginRendering(const VkCommandBuffer& commandBuffer, VkRenderingInfo* renderingInfo) {
	vkCmdBeginRendering(commandBuffer, renderingInfo);
}

void VkUtil::CommandBufferEndRendering(const VkCommandBuffer& commandBuffer) {
	vkCmdEndRendering(commandBuffer);
}

void VkUtil::CommandBufferSetScissorAndViewport(const VkCommandBuffer& commandBuffer, float width, float height, float x, float y, float minDepth, float maxDepth) {
	VkRect2D scissor{};
...
VkRenderingAttachmentInfo VkUtil::RenderPassGetColorAttachmentInfo(const VkImageView view, VkClearValue* clear, VkImageLayout layout, VkResolveModeFlagBits resolveMode, VkImageView resolveImage, VkImageLayout resolveLayout) {
...
VkRenderingAttachmentInfo VkUtil::RenderPassGetDepthAttachmentInfo(const VkImageView view, VkImageLayout layout) {
...
VkRenderingInfo VkUtil::RenderPassCreateRenderingInfo(VkExtent2D renderExtent, VkRenderingAttachmentInfo* colorAttachments, VkRenderingAttachmentInfo* depthAttachment, uint32_t colorAttachmentCount) {
...
VkPipelineMultisampleStateCreateInfo VkUtil::PipelineDefaultMultiSampleState() {
...
VkPipelineInputAssemblyStateCreateInfo VkUtil::PipelineDefaultInputAssemblyState() {
...
VkPipelineRasterizationStateCreateInfo VkUtil::PipelineDefaultRasterizationState() {
...
VkPipelineDepthStencilStateCreateInfo VkUtil::PipelineDefaultDepthStencilState() {
...
VkPipelineDynamicStateCreateInfo VkUtil::PipelineDefaultDynamicState() {
...
VkPipelineColorBlendAttachmentState VkUtil::PipelineDefaultColorBlendAttachmentState() {
...
VkSampler VkUtil::CreateSampler(const VkDevice& device, VkSamplerAddressMode addressMode, VkFilter filter, float anisotropy, float minLod, float maxLod)
{
